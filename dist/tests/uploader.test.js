...
function createXhrRequest(name, body, declaredSize = Buffer.byteLength(body)) {
    ...
    request.header = (headerName) => {
        ...
        if (normalized === 'x-file-name')
            return name;
        if (normalized === 'x-file-size')
            return String(declaredSize);
        return null;
    };
    return request;
}
...
(0, vitest_1.describe)('Uploader', () => {
    ...
    (0, vitest_1.it)('sanitizes direct XHR upload names before moving files', async () => {
        ...
        (0, vitest_1.expect)(fs.readFileSync(path.join(root, 'public', 'files', 'escape.txt'), 'utf8')).toBe('xhr payload');
        (0, vitest_1.expect)(fs.existsSync(path.join(root, 'public', 'escape.txt'))).toBe(false);
    });
    (0, vitest_1.it)('rejects direct XHR uploads whose streamed bytes exceed the configured limit', async () => {
        const root = createTempRoot();
        const uploader = createUploader(root, {
            validate: true,
            maxFileSize: 4,
    ...
    (0, vitest_1.it)('reserves safe names for duplicate files in the same request', async () => {
        ...
        (0, vitest_1.expect)(Array.isArray(result)).toBe(true);
        const files = result;
        (0, vitest_1.expect)(files.map((file) => file.name).sort()).toEqual(['duplicate.txt', 'duplicate_1.txt']);
        (0, vitest_1.expect)(fs.readFileSync(path.join(root, 'public', 'files', 'duplicate.txt'), 'utf8')).toBe('first');
        (0, vitest_1.expect)(fs.readFileSync(path.join(root, 'public', 'files', 'duplicate_1.txt'), 'utf8')).toBe('second');
    });
    (0, vitest_1.it)('normalizes Multer-style file arrays', async () => {
        ...
        (0, vitest_1.expect)(fs.readFileSync(path.join(root, 'public', 'files', 'multer-name.txt'), 'utf8')).toBe('multer payload');
    });
    ...
    (0, vitest_1.it)('reports Invalid upload headers when XHR x-file-name is missing', async () => {
        const root = createTempRoot();
        const uploader = createUploader(root);
        const request = stream_1.Readable.from([Buffer.from('payload')]);
        request.files = {};
        request.xhr = true;
        request.header = () => null;
        ...
        (0, vitest_1.expect)(Array.isArray(result)).toBe(false);
        const file = result;
        (0, vitest_1.expect)(file.error).toBe('Invalid upload headers');
    });
    (0, vitest_1.it)('reports Invalid upload headers when XHR x-file-size is non-numeric', async () => {
        const root = createTempRoot();
        const uploader = createUploader(root);
        const request = stream_1.Readable.from([Buffer.from('payload')]);
        request.files = {};
        request.xhr = true;
        request.header = (name) => {
            if (name.toLowerCase() === 'x-file-name')
                return 'broken.txt';
            if (name.toLowerCase() === 'x-file-size')
                return 'not-a-number';
            return null;
        };
        const result = await uploadFile(uploader, request);
    ...
    (0, vitest_1.it)('returns a validation error for oversized files on the form path', async () => {
        const root = createTempRoot();
        ...
        (0, vitest_1.expect)(fs.existsSync(path.join(root, 'public', 'files', 'oversized.txt'))).toBe(false);
    });
    (0, vitest_1.it)('keeps numbering safe names beyond the first collision', async () => {
        const root = createTempRoot();
        const uploader = createUploader(root);
        ...
        (0, vitest_1.expect)(Array.isArray(result)).toBe(true);
        const [file] = result;
        (0, vitest_1.expect)(file.name).toBe('avatar_10.txt');
        (0, vitest_1.expect)(fs.readFileSync(path.join(uploadDir, 'avatar_10.txt'), 'utf8')).toBe('new');
    });
    (0, vitest_1.it)('accepts a single Multer file passed under req.files directly', async () => {
        const root = createTempRoot();
        const uploader = createUploader(root);
        ...
        (0, vitest_1.expect)(fs.readFileSync(path.join(root, 'public', 'files', 'single.txt'), 'utf8')).toBe('one shot');
    });
});
